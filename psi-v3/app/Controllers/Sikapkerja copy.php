...
class Sikapkerja extends BaseController
{
	...
	public function petunjuksoal() {
        ...
        $this->session->set("sk_group_id",$sk_group_id);
        $resused = $this->soalmodel->getMaxUsed($this->session->user_id)->getResult();
        if (count($resused)>0) {
            $used = $resused[0]->maxused + 1;
        } else {
            $used = 1;
        }
       ...
       <div>
        ...
        <div style='margin-top:20px;text-align:cennter;'><button style='font-size: 30px;' class='btn btn-primary' onclick='startujiansk(\"$class_soal\",1,1,$used,\"null\",\"null\")'>Mulai</button></div>
    ...
    public function startujian() {
        ...
        $res = $this->soalmodel->getSoalSK($no_soal,4,5,$kolom_id,$this->session->get("sk_group_id"))->getResult();
        if (count($res)>0) {
            $res_soal = $res;
        } else {
            $no_soal = 1;
            ...
            $ret = "<div class='col-lg-12' style='color:#000000;'>
                        <a href='".base_url()."/home'><button class='btn btn-primary'>Menu Utama</button></a>
                            ...
                            <div style='width:100%;text-align:center;'>
                                ...
                                $salah = 0;
                                $soal_terjawab = 0;
                                    $res_responSK = $this->soalmodel->getResponSK($this->session->user_id,$this->session->session,$key->kolom_id,5,$used,$this->session->sk_group_id)->getResult();
                ...
                $ret .= "</div>";
        } else if ($class_soal == "rehatsk" && $kolom_id == 10) {
            $kolom_nm = "";
            $soal_terjawab_chart = "";
            $jawaban_benar_chart = "";
            $ret = "<div class='col-lg-12' style='text-align:center;min-height: 400px;s'>
                    <h1 style='margin:10px;'>Selesai, proses perhitungan hasil . . .</h1>
