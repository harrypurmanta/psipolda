...
<body class="hold-transition skin-blue layout-top-nav">
    ...
    function updateFinishRespon(materi_id,group_id) {
        $.ajax({
            url: "<?= base_url('dass/updateFinishRespon') ?>",
            type: "post",
            dataType: "json",
            data: {
                "materi_id": materi_id,
                    ...
                    updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                window.location.href = "<?= base_url() ?>/dass/hasiltryout/"+<?= $request->uri->getSegment(3) ?>+"/" +<?= $request->uri->getSegment(4) ?>;
                            }
                        });
