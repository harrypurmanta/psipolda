...
<body class="hold-transition skin-blue layout-top-nav">
    ...
    function updateFinishRespon(materi_id,group_id) {
        $.ajax({
            url: "<?= base_url('materiG/updateFinishRespon') ?>",
            type: "post",
            dataType: "json",
            data: {
                "materi_id": materi_id,
    ...
    function startujian(proc,jawaban_id,pilihan_nm) {
        ...
        $.ajax({
            url: "<?= base_url('materiG/materigujian') ?>",
            type: "post",
            dataType: "json",
            data: {
                    ...
                    updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                                window.location.href = "<?= base_url() ?>/materiG/hasiltryout/"+<?= $request->uri->getSegment(3) ?>+"/" +<?= $request->uri->getSegment(4) ?>;
                            }
                        });
