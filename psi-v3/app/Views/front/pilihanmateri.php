                    ...
                    <div class="col-md-12">
                            <?php
                                ...
                                foreach ($group as $key) {
                                    $query = $db->query("SELECT * FROM respon WHERE materi = $key->materi_id AND group_id = $key->group_soal_id AND created_user_id = $user_id AND status_cd = 'finish'")->getResultArray();
                                    
                                        if (count($query)>0) {
                                            if ($key->group_soal_id != 5) {
                                                $click = "";
                                                $class_bg = "bg-green";
                                                $a_bg = "bg-green";
                                                $icon= "fa-check";
                                                ...
                                                $click = "onclick='showtoken(".$key->group_soal_id.", ".$key->materi_id.")'";
                                                $class_bg = "bg-gray";
                                                $a_bg = "bg-blue";
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data == "sukses") {
                        ...
                        if (group_id == 3) {
                            window.location.href = "<?= base_url() ?>/kreplin/petunjukkreplin/"+materi_id+"/"+group_id;
                        } else if (group_id == 2) {
                            window.location.href = "<?= base_url() ?>/tryout/petunjukkedua/"+materi_id+"/"+group_id;
                        } else if (group_id == 1) {
                            window.location.href = "<?= base_url() ?>/tryout/petunjukpertama/"+materi_id+"/"+group_id;
                        } else if (group_id == 4) {
                            window.location.href = "<?= base_url() ?>/dass/petunjukdass/"+materi_id+"/"+group_id;
                        } else if (group_id == 5) {
                            window.location.href = "<?= base_url() ?>/tiu5/index/"+materi_id+"/"+group_id;
                        } else if (group_id == 6) {
                            window.location.href = "<?= base_url() ?>/papi/petunjukpapi/"+materi_id+"/"+group_id;
                        } else if (group_id == 7) {
