...
<body class="hold-transition skin-blue layout-top-nav">
    ...
    function updateFinishRespon(materi_id, group_id) {
        $.ajax({
            url: "<?= base_url('pkp/updateFinishRespon') ?>",
            type: "post",
            dataType: "json",
            data: {
                "materi_id": materi_id,
    ...
    function startujian(proc,jawaban_id,pilihan_nm) {
        ...
        let sk_group_id = <?= $request->uri->getSegment(5) ?>;
        let waktu = document.getElementById('countdown').textContent;
        $.ajax({
            url: "<?= base_url('pkp/pkpujian') ?>",
            type: "post",
            dataType: "json",
            data: {
            ...
            success: function(data) {
                if (data.proc == "selesai") {
                    ...
                    if (<?= $request->uri->getSegment(5) ?> == 1) {
                        Swal.fire({
                                title: "anda telah selesai mengerjakan Test PKP 1, klik Ok untuk lanjut ke PKP 2",
                                icon: "info",
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    window.location.href = "<?= base_url() ?>/pkp/petunjukpkp/"+<?= $request->uri->getSegment(3) ?>+"/" +<?= $request->uri->getSegment(4) ?>+"/2";
                                }
                            });
                    } else if (<?= $request->uri->getSegment(5) ?> == 2) {
                        Swal.fire({
                                title: "anda telah selesai mengerjakan Test PKP 2, klik Ok untuk lanjut ke PKP 3",
                                icon: "info",
                                showCancelButton: false,
                                confirmButtonColor: "#3085d6",
                                cancelButtonColor: "#d33",
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    window.location.href = "<?= base_url() ?>/pkp/petunjukpkp/"+<?= $request->uri->getSegment(3) ?>+"/" +<?= $request->uri->getSegment(4) ?>+"/3";
                                }
                            });
                    } else if (<?= $request->uri->getSegment(5) ?> == 3) {
                        updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    ...
                                    updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                                    window.location.href = "<?= base_url() ?>/pkp/hasiltryout/"+<?= $request->uri->getSegment(3) ?>+"/" +<?= $request->uri->getSegment(4) ?>;
                                }
                            });
                    }
                } else {
