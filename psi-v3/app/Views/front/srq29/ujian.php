...
<body class="hold-transition skin-blue layout-top-nav">
    ...
    function updateFinishRespon(materi_id,group_id) {
        $.ajax({
            url: "<?= base_url('srq29/updateFinishRespon') ?>",
            type: "post",
            dataType: "json",
            data: {
                "materi_id": materi_id,
                    ...
                    updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                updateFinishRespon(<?= $request->uri->getSegment(3) ?>,<?= $request->uri->getSegment(4) ?>);
                                window.location.href = "<?= base_url() ?>/srq29/hasiltryout/"+<?= $request->uri->getSegment(3) ?>+"/" +<?= $request->uri->getSegment(4) ?>;
                            }
                        });
